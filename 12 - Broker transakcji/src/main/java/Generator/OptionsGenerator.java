...
public class OptionsGenerator {
    ...
    public Options fillOptoins(Options options){
        ...
        options.addOption("queue", "queue", true, "Nazwa kolejki");
        options.addOption("broker", "broker", true, "Adres brokera");
        options.addOption("topic", "topic", true, "Nazwa tematu");
        return options;
    }
    ...
    public Transaction fillTransactionFromFile(Transaction transactions, String [] args){
        ...
        for (int i = 0; i < args.length; i++){
            ...
            if (args[i].equals("-itemsFile")){
                ...
                transaction.setQueueName(args[++i]);
                continue;
            }
            else if (args[i].equals("-broker")){
                transaction.setBroker(args[++i]);
                continue;
            }
            else if (args[i].equals("-topic")){
                transaction.setTopic(args[++i]);
                continue;
            }
        }
        return transaction;
    ...
    public Transaction fillTransaction(Transaction transaction, CommandLine cmd){
        ...
        if (cmd.hasOption("broker"))
            transaction.setBroker(cmd.getOptionValue("broker"));
        if (cmd.hasOption("queue"))
            transaction.setQueueName(cmd.getOptionValue("queue"));
        if (cmd.hasOption("topic"))
            transaction.setTopic(cmd.getOptionValue("topic"));
